...
> This is the original README describing a single host
> running services on local ports, exposed to the Tailnet by **one** server-side
> `tailscale serve` process. The infrastructure has since changed drastically.

## Comparison between this approach and the sidecar one

| Aspect | Serve Routing (this doc) | Sidecar (project [`README.md`](../README.md)) |
|---|---|---|
| Exposure | Apps bind `127.0.0.1:<port>` on the host; one host-level `sudo tailscale serve` per app on different HTTPS ports | Every app gets its **own** `tailscale/tailscale` sidecar container = its own Tailnet node, IP, MagicDNS name, cert |
| Host ports | Each app mapped a host port | **Only** `homepage` binds a host port (`127.0.0.1:3000`). All other apps have **no** host ports |
| Tailscale | Installed on the host, `tailscaled` as a system daemon, `sudo tailscale up` | No host Tailscale needed for the apps; each sidecar runs `tailscaled` in **userspace** mode inside the container |
| Routing | `sudo tailscale serve --https=443 localhost:3000` etc. (manual, host-side) | Declarative per-app `tailscale-serve/<app>.json` mounted into each sidecar via `TS_SERVE_CONFIG` |
| URLs | `https://host.<tailnet>.ts.net:443/8443/9443` (port-per-app on one node) | `https://<app>.<tailnet>.ts.net` (clean per-app hostname, all on :443) |
| Apps | Silverbullet, Forgejo, Anki Sync | + Vaultwarden, Stirling PDF, Actual Budget, Syncthing, Homepage dashboard |
| Data ownership | Manual `sudo chown -R <uid>:<gid>` after finding the container UID | Apps run as **root in-container**; default rootless Podman maps that to the host user — no chown |
